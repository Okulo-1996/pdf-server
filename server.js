...
// ========== STAMP FUNCTION ==========
function addStampToPDF(doc, stampPath) {
    try {
        // Check if stamp file exists
        if (fs.existsSync(stampPath)) {
            // Add stamp to bottom right corner
            doc.image(stampPath, {
                fit: [100, 100],  // Size of stamp (width, height)
                align: 'right',
                valign: 'bottom'
            });
            console.log("✅ Stamp added successfully");
        } else {
            console.log("⚠️ Stamp file not found at:", stampPath);
        }
    } catch (error) {
        console.log("⚠️ Could not add stamp:", error.message);
    }
}
